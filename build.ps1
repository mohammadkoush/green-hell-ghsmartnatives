# Build GHSmartNatives.dll and (optionally) deploy it into Green Hell's BepInEx plugins folder.
#
...
$outDll  = Join-Path $outDir 'GHSmartNatives.dll'
...
Write-Host "Compiling GHSmartNatives..." -ForegroundColor Cyan
& $csc $argList.ToArray()
...
$dest = Join-Path $GameDir 'BepInEx\plugins\GHSmartNatives'
...
try {
    Copy-Item $outDll (Join-Path $dest 'GHSmartNatives.dll') -Force -ErrorAction Stop
} catch {
    Write-Host ""
...
Write-Host "" 
Write-Host "J opens the natives panel: hunt, roam and savages." -ForegroundColor Yellow
Write-Host ""
